...
def OptionPicker(kanaal):
	...
	oc.add(DirectoryObject(key = Callback(NameList, kanaal=kanaal), title=L('Programs by name'), thumb=R(CHANNELS[kanaal]['icon']), art=R(CHANNELS[kanaal]['art'])))
	oc.add(DirectoryObject(key = Callback(GenreList, kanaal=kanaal), title=L('Programs by genre'), thumb=R(CHANNELS[kanaal]['icon']), art=R(CHANNELS[kanaal]['art'])))
...
def NameList(kanaal):
	...
	oc.add(DirectoryObject(key = Callback(Episode, title='0-9', kanaal=kanaal, function='name'), title=L('09'), art=R(CHANNELS[kanaal]['art'])))
	oc.add(DirectoryObject(key = Callback(Episode, title='ABC', kanaal=kanaal, function='name'), title=L('ABC'), art=R(CHANNELS[kanaal]['art'])))
	oc.add(DirectoryObject(key = Callback(Episode, title='DEF', kanaal=kanaal, function='name'), title=L('DEF'), art=R(CHANNELS[kanaal]['art'])))
	oc.add(DirectoryObject(key = Callback(Episode, title='GHI', kanaal=kanaal, function='name'), title=L('GHI'), art=R(CHANNELS[kanaal]['art'])))
	oc.add(DirectoryObject(key = Callback(Episode, title='JKL', kanaal=kanaal, function='name'), title=L('JKL'), art=R(CHANNELS[kanaal]['art'])))
	oc.add(DirectoryObject(key = Callback(Episode, title='MNO', kanaal=kanaal, function='name'), title=L('MNO'), art=R(CHANNELS[kanaal]['art'])))
	oc.add(DirectoryObject(key = Callback(Episode, title='PQR', kanaal=kanaal, function='name'), title=L('PQR'), art=R(CHANNELS[kanaal]['art'])))
	oc.add(DirectoryObject(key = Callback(Episode, title='STUV', kanaal=kanaal, function='name'), title=L('STUV'), art=R(CHANNELS[kanaal]['art'])))
	oc.add(DirectoryObject(key = Callback(Episode, title='WXYZ', kanaal=kanaal, function='name'), title=L('WXYZ'), art=R(CHANNELS[kanaal]['art'])))
...
def GenreList(kanaal):
	...
	oc.add(DirectoryObject(key = Callback(Episode, title='13', kanaal=kanaal, function='genre'), title=L('Entertainment'), art=R(CHANNELS[kanaal]['art'])))
	...
	oc.add(DirectoryObject(key = Callback(Episode, title='10', kanaal=kanaal, function='genre'), title=L('Documentary'), art=R(CHANNELS[kanaal]['art'])))
